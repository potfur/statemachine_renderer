...
class Renderer
{
     ...
     * @return string
     */
    public function png(Document $document, $outputFile): string
    {
     ...
     * @return string
     */
    public function svg(Document $document, $outputFile): string
    {
     ...
     */
    private function exec(Document $document, $outputFile, $format): string
    {
        $dotFile = $this->buildDotFile($document);

        $output = [];
        $status = 0;

        exec(
            sprintf('%s -T%s %s -o%s', $this->executable, $format, $dotFile, $outputFile),
            $output,
            $status
        );
     ...
     * @return string
     */
    private function buildDotFile(Document $document): string
    {
